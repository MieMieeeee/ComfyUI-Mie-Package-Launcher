...
def _format_update_summary(core_res, req_res):
    ...
    实现已搬到 :mod:`core.update_summary`（纯函数模块，不依赖 Qt）。这里保留
    模块级名字是向后兼容：``ui_qt/qt_app.py`` 内部和老测试都通过这个名字引用。
    抽离原因是本模块在部分 PyQt5/sip ABI 组合下 import 时段错误，导致连这个
    纯函数都测不到；搬到 ``core`` 子包后测试可绕开段错误路径。
    """
    from core.update_summary import format_update_summary

    return format_update_summary(core_res, req_res)
...
def _confirm_deps_or_warn(parent, auto_update_deps_var) -> bool:
    ...
    实现已搬到 :mod:`core.update_summary`（纯函数模块，``DialogHelper`` lazy import）。
    这里保留为向后兼容别名。抽离原因同 :func:`_format_update_summary`。
    """
    from core.update_summary import confirm_deps_or_warn

    return confirm_deps_or_warn(parent, auto_update_deps_var)
...
class PyQtLauncher(QtWidgets.QMainWindow, process_events.ProcessCallback):
    ...
    def _setup_ui(self):
        ...
        _scale = self._scale  # 仅供本方法体内裸 _sp/_st 调用快路径使用
        # self._sp / self._st 必须**读 self._scale（live）**，不能闭包捕获上面的
        # 局部 _scale —— 否则屏幕切换后 _apply_screen_change 更新 self._scale，
        # 这俩 helper 仍返回构造时的旧值（review 标的回归隐患）。
        self._sp = lambda base: max(1, int(round(base * self._scale)))  # scaled px
        self._st = lambda base: max(6, int(round(base * self._scale)))  # scaled pt
