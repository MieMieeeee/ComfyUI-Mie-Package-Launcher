...
- 启动后约 1 秒，若存在可用公告将弹出聚合滚动弹窗；点击“知道了/不再弹出”分别记录已读与屏蔽。
...
- `services/announcement_service.py`：远程公告抓取、清单聚合、版本规则匹配、弹窗显示与已读/屏蔽持久化。
...
- 公告调度：入口 UI 构建后延时调用 `services.announcement.show_if_available()`；公告内容写入缓存并在“关于启动器”页面可查看。
